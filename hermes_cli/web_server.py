...
# ── /v1/chat/completions — SSE proxy to gateway ──────────────────────────────

_GATEWAY_API = os.environ.get("HERMES_GATEWAY_URL", "http://127.0.0.1:8642")

@app.post("/v1/chat/completions")
async def chat_completions_proxy(request: Request):
    """Proxy streaming chat completions to the running Hermes gateway."""
    import http.client
    import socket
    import urllib.parse

    body_bytes = await request.body()
    parsed = urllib.parse.urlparse(_GATEWAY_API)
    ...
    def _stream():
        try:
            ...
            # If non-200, wrap error as SSE event so the frontend can display it
            if resp.status != 200:
